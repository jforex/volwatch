...
export default function SurfacePage() {
 ...
 const [liveNow, setLiveNow] = useState(Date.now());
  ...
  // When scrubbing, freeze "now" to the scrubbed timestamp.
  // Otherwise use live wall clock.
  ...
  return (
    <main className="px-3 sm:px-5 py-3 sm:py-4">
     ...
     {/* Time travel */}
        ...
        {/* Thin historical coverage banner */}
        {isScrubbing && expiryCount < 3 && (
          <div className="mt-3 rounded border border-amber-500/40 bg-amber-950/30 px-4 py-3">
            <div className="flex items-start gap-3">
              <span className="h-2 w-2 rounded-full bg-amber-500 mt-1.5 shrink-0" />
              <div className="min-w-0">
                <p className="font-mono text-xs uppercase tracking-widest text-amber-300 font-bold">
                  THIN HISTORICAL COVERAGE
                </p>
                <p className="mt-1 font-mono text-xs text-neutral-300 leading-relaxed">
                  {expiryCount === 0
                    ? "No oracles had complete state at this timestamp. Predict oracles bootstrap through several events before SVI calibration arrives."
                    : `Only ${expiryCount} oracle${expiryCount === 1 ? "" : "s"} had complete state at this timestamp. Other oracles were still bootstrapping or had already settled.`}
                </p>
                <p className="mt-1.5 font-mono text-xs text-neutral-300">
                  Scrub closer to now or click "Live" for full coverage.
                </p>
              </div>
            </div>
          </div>
        ...
        <section className="col-span-12 lg:col-span-8 rounded border border-neutral-800 bg-neutral-900 min-h-[520px] overflow-hidden">
            <div className="border-b border-neutral-800 px-4 py-2">
              <div className="flex items-center justify-between">
                ...
                <span className="font-mono text-xs text-neutral-400">strike × expiry → IV (thermal)</span>
              </div>
        ...
        <section className="mt-3 rounded border border-neutral-800 bg-neutral-900">
          <div className="flex items-center justify-between border-b border-neutral-800 px-4 py-2">
            <span className="font-mono text-xs uppercase tracking-widest text-neutral-300">ARBITRAGE CHECK</span>
            <span className="font-mono text-xs text-neutral-200">Gatheral butterfly · calendar monotonicity</span>
          </div>
        ...
        <div className="mt-3 grid grid-cols-12 gap-3">
          <section className="col-span-12 lg:col-span-4 rounded border border-neutral-800 bg-neutral-900">
            <div className="flex items-center justify-between border-b border-neutral-800 px-4 py-2">
              <span className="font-mono text-xs uppercase tracking-widest text-neutral-300">ORACLES</span>
              <span className="font-mono text-xs text-neutral-200">{Object.keys(oracles).length} total</span>
            </div>
            <div className="p-2">
          ...
          <section className="col-span-12 lg:col-span-8 rounded border border-neutral-800 bg-neutral-900">
            <div className="flex items-center justify-between border-b border-neutral-800 px-4 py-2 gap-3">
              <div className="flex items-center gap-3 min-w-0">
                ...
                {selectedShape && (
                  <span className={`inline-flex items-center gap-1.5 rounded border px-2 py-0.5 font-mono text-xs uppercase tracking-widest font-bold ${
...
function PlaceholderSlot({ label, sub }: { label: string; sub: string }) {
  return (
    <div className="flex h-full min-h-[180px] flex-col items-center justify-center gap-1 p-6 text-center">
      <span className="font-mono text-xs text-neutral-300">{label}</span>
      <span className="font-mono text-xs text-neutral-200">{sub}</span>
    </div>
  );
}