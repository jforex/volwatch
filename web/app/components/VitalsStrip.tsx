      ...
      <div className="mb-4 flex items-baseline justify-between">
        <div>
          ...
          <p className="mt-1 text-xs text-neutral-500">
            At-a-glance status across the protocol. Read the bars.
          </p>
        </div>
      </div>
...
function VitalCard({ v }: { v: Vital }) {
  ...
  return (
    <div className={`rounded-lg border ${palette.border} ${palette.bg} p-4`}>
      ...
      <p className={`mt-2 text-lg font-bold ${palette.statusText}`}>
        {v.status}
      </p>
      ...
      <p className="mt-3 text-xs leading-relaxed text-neutral-600">{v.detail}</p>
    </div>
  );
}
